...
def input_language() -> str:
    ...
    languages_choices = [{"name": f"{name} ({code})", "value": code} for code, name in LANGUAGES.items()]
    try:
        questions = [
            inquirer.List(
                'language_choices',
        ...
        return next(lang['value'] for lang in languages_choices if lang['name'] == answers['language_choices'])
    except (KeyboardInterrupt, TypeError):
        print(color_text(f'🚩 {_('Process interrupted. Exiting...')}', 'red'))
        sys.exit(0)
    except Exception as error:
        print(color_text(f'❌ {_('Unexpected error occurred')}: {error}', 'red'))
        sys.exit(1)
        ...
        match args[0]:
            case "addlanguage":
                ...
                if not os.path.exists(os.path.join(LOCALES_DIR, language, "LC_MESSAGES", "messages.po")):
                    ...
                    # Creating .mo file
                    mo_file: str = os.path.join(LOCALES_DIR, language, "LC_MESSAGES", "messages.mo")
            ...
            case "update":
                ...
                if os.path.exists(find_file("messages.po", LOCALES_DIR,)):
                    ...
                    os.system(f"cp {POT_FILE} {os.path.join(LOCALES_DIR, 'messages.po')}")
                ...
                if find_file("messages.po~", LOCALES_DIR):
                    list_files: list = find_file("messages.po~", LOCALES_DIR, list_return=True)
