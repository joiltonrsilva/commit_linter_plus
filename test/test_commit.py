import pytest
import subprocess
from task_commit.utils import color_text, get_git_user, check_git_status, is_git_flow, get_current_branch, create_commit, execute_push


# Testando a função color_text
@pytest.mark.parametrize("text, color, expected_output", [
...
def test_is_git_flow(mocker):
    mocker.patch('subprocess.check_output', side_effect=subprocess.CalledProcessError(1, 'git flow config'))
...
def test_get_current_branch(mocker):
    ...
    mocker.patch('subprocess.check_output', side_effect=subprocess.CalledProcessError(1, 'git rev-parse'))
...
def test_create_commit(mocker):
    ...
    subprocess.run.assert_called_once_with(
        ["git", "commit", "-m", "feat(core): implement new feature (👤 user: testuser)"], check=True
    )
...
def test_execute_push(mocker):
    ...
    subprocess.run.assert_called_once_with(["git", "push", "origin", branch], check=True)
